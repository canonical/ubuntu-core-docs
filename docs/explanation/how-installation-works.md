...
The first three (1-3) are also described in the [recovery mode](recovery-modes) boot process:

1. Boot through the bootloader, its assets, and boot configuration in `ubuntu-seed`.
1. Bootloader loads the kernel from a designated [recovery system](recovery-modes).
...
A [gadget](/reference/gadget-snap-format) can optionally define an _install-device hook_ . This is invoked from **install mode** before rebooting into **run mode**, between steps 4 and 5 above, and also after a [factory reset](recovery-modes.md#factory-reset), to perform early hardware and firmware configuration.
...
See [Building the image](/how-to-guides/image-creation/index) for details on how images are built.
