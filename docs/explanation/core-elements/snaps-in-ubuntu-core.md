...
The [kernel snap](https://snapcraft.io/docs/kernel-snap) is selected with the {ref}`model assertion <reference-assertions-model>` describing the device which is produced and signed before the image is built. Once the image is built, the kernel snap may be updated at any time. Note that the only way to change to a different kernel snap (i.e. different name/ID) is to {ref}`remodel <explanation-remodeling>`.
