...
1. **Gadget snap with prepare-device hook pointing at Serial vault** 
  The system needs a {ref}`gadget snap <reference-gadget-snap-format>` that has a {ref}`prepare-device hook <ref-gadget-snap-format_prepare-device-hook>` script that points the device to the Serial Vault. The hook script sets a few snapd variables, including the Serial vault URL and device’s serial number.
