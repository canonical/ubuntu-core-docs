...
With up to 12 years of security maintenance, Ubuntu Core is a minimal, immutable OS that enables developers to [build](https://documentation.ubuntu.com/core/tutorials/build-your-first-image/#ref-index-build-your-first-image) and [deploy](https://documentation.ubuntu.com/core/how-to-guides/deploy-an-image/) production-grade images for embedded devices on various architectures at any scale.
...
Ubuntu Core 26 is using a new Chisel-based build system that brings precise composition using a process that will transform how future Core base snaps will be assembled. It relies on Canonical’s release-specific package slice definitions, enforcing explicit, traceable dependencies.
...
15 years of support.
Graham to test Gabriel pull request.
